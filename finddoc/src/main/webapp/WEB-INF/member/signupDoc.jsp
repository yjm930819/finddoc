						...
						$("#hnameb")
								.click(
										function() {
											hspname = $("#hname").val();
											haddr = $("#haddr").val();
											var url = "/finddoc/member/hnameSearchPopup.do?hospname="
													+ encodeURI(hspname)
													+ "&pageno="
													+ 1
													+ "&rows="
													+ 10 + "&haddr=" + haddr; //팝업창 페이지 URL
											var popupOption = "width=600,height=700,location=no,status=no,toolbars=no,top=70,left=800"; //팝업창 옵션(optoin)
											openWin = window.open(url, "child",
													popupOption);
											/* 	$.ajax({
...
<body>
	...
	<div class="container-fluid" id="con1">
		<form:form commandName="hadminDTO" role="form" class="form-horizontal"
			name="myform" method="post" action="/finddoc/member/insert.do">
			...
			<div class="form-group">
				...
				<div class="col-sm-1">
					<input type="button" id="hnameb"
						class="btn btn-round btn-primary form-control" value="검색">
				</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					<input type="password" id="pw" name="pw" placeholder="비밀번호"
						class="form-control"> <span>비밀번호는 8~10글자
						영문자,숫자,특수기호(!@#$%^&)반드시 포함</span> <span class="help-block"
						style="color: red"><form:errors path="pw" /></span>
				</div>
			</div>
			<div class="form-group">
